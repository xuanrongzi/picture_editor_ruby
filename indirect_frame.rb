class IndirectFrame
  ...
  def initialize(frame_source, x_offset, y_offset, height, width)
    ...
    raise Exception "Indirect Frame out bound" if (@x_off+@height>@source.hejght)||(@y_off+@width>@source.width)
  end
  ...
  def []=(row,col,pixel)
    ...
    @source[@x_off+i,@y_off+j]=pixel if ((row>=0)|(col>=0)|(row<@height)|(col<@width))
  end
  alias put []=
